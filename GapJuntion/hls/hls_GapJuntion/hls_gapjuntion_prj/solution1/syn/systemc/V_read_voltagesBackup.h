...
#ifndef _V_read_voltagesBackup_H_
#define _V_read_voltagesBackup_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "V_read_voltagesBackup_memcore.h"

SC_MODULE(V_read_voltagesBackup) {
    static const unsigned int DataWidth    = 32;
    static const unsigned int AddressRange = 32;
    static const unsigned int AddressWidth = 14;
...
protected:
    ...
    // buffer instances
    V_read_voltagesBackup_memcore* buffer[BufferCount];
...
public:
    ~V_read_voltagesBackup();
    SC_CTOR(V_read_voltagesBackup) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "V_read_voltagesBackup_memcore_U_";
            name += szTmp;

            buffer[i] = new V_read_voltagesBackup_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "V_read_voltagesBackup.trace");
            // ports
