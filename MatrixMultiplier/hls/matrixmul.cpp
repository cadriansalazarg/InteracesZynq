...
void productor(hls::stream<data_type> &bus_local, hls::stream<packaging_data>& in_fifo);
void consumidor(hls::stream<data_type> &bus_local, hls::stream<packaging_data>& out_fifo);
...
void Wrapper_Matrix_Multiplier(hls::stream<packaging_data>& in_fifo, hls::stream<packaging_data>& out_fifo){
	...
	productor(bus_local, in_fifo);
	consumidor(bus_local, out_fifo);
}
...
void consumidor(hls::stream<data_type> &bus_local, hls::stream<packaging_data>& out_fifo) {
	...
	output_packet.FPGA_ID = 0x00; // 8 bits
	output_packet.TX_UID = 0x00; // 8 bits
	output_packet.RX_UID = 0x00; // 8 bits
